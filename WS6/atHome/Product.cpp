...
namespace AMA
{
	...
	std::fstream &Product::load(std::fstream &file)
	{
		if (file.is_open())
		{
			...
			file.ignore(1, ',');
			//file >> newNameOfProduct;
			file.getline(newNameOfProduct, max_name_length, ',');
			file.ignore(1, ',');
			//file >> newProductUnit;
	...
	std::ostream &Product::write(std::ostream &os, bool linear) const
	{
		...
		else if (!isEmpty())
		{
			if (linear) //
			{
				os << std::setw(max_sku_length) << std::left << productSKU << "|" << std::setw(20) << std::left << nameOfProduct << "|" << std::setw(7) << std::fixed << std::setprecision(2) << std::right << cost() << "|" << std::setw(4) << std::right << quantityOnHand << "|" << std::setw(10) << std::left << productUnit << "|" << std::setw(4) << std::right << quantityNeeded << "|"; //problem is here [ms5]
			}
